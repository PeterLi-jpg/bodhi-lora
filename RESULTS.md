...
**Data split (enforced — see [#3](https://github.com/PeterLi-jpg/bohdi-lora/issues/3), [#60](https://github.com/PeterLi-jpg/bohdi-lora/issues/60)):**
- **Training pool:** HealthBench Full (5000 examples) minus *all* HealthBench Hard prompts (Hard ⊂ Full) → ~4000 unique non-Hard prompts. Run through the BOHDI wrapper, graded, and filtered to produce `data/sft/train.jsonl`. Trace generation passes `--exclude-ids data/raw/healthbench_hard.jsonl data/raw/hard_200_sample_ids.json`; `filter_traces.py` re-applies the same exclusion as a defensive belt-and-suspenders against stale resume files.
- **Test set:** 5 independent 200-sample bootstrap draws from HealthBench Hard (1000 examples), one per seed. Each seed's draw is generated by `scripts/make_bootstrap_eval_ids.py` (deterministic in the seed) and written to `data/raw/hard_seed_<SEED>.json`. Because all 1K Hard are held out from training, every draw is honestly out-of-sample. Scores are reported as mean ± std (and percentile 95% CI) across the 5 draws — variance captures both training noise and eval-set-choice noise.
- **Leakage prevention:** the launchers run a preflight gate before training that aborts on any HealthBench Hard prompt found in `data/sft/train.jsonl`:
  ```
  python scripts/check_dataset_overlap.py \
      --train-jsonl data/sft/train.jsonl \
      --tag-overlap
  ```
  This checks (1) no fixed-eval IDs in the training file, (2) **no Hard prompts at all in the training file** (post-#60 invariant), and (3) tag/theme distribution is balanced. Pass `--seeds 5` to also walk the per-seed bootstrap draws and confirm none leak. Do not report any number from a single-seed draw.
