...
if [ "$BENCH" = "healthbench" ]; then
    ...
    if [ ! -s "$BENCH_JSONL" ]; then
        python scripts/build_benchmark_jsonl.py --benchmark "$BENCH" --out "$BENCH_JSONL" --max "$((GEN_MAX + EVAL_HOLDOUT_N + 500))"
    fi
    SAMPLE_IDS="${WORK}/eval_ids.json"
    if [ ! -s "$SAMPLE_IDS" ]; then       # deterministic holdout = first N prompt_ids
        python - "$BENCH_JSONL" "$SAMPLE_IDS" "$EVAL_HOLDOUT_N" <<'PY'
import json, sys
...
if [ ! -s "$RAW" ]; then
    ...
    echo "[gen] GPU $GPU"
    CUDA_VISIBLE_DEVICES="$GPU" python scripts/generate_traces.py \
...
if [ ! -s "${SFT}/train.jsonl" ] || [ ! -s "${SFT}/val.jsonl" ]; then
    ...
    echo "[filter] GPU $GPU"
    CUDA_VISIBLE_DEVICES="$GPU" python scripts/filter_traces.py \
...
run_seed() {   # $1=seed  $2=gpu
    ...
    CUDA_VISIBLE_DEVICES="$GPU" python scripts/train_lora.py \
    ...
    for spec in "base_no_wrapper::" "base_bodhi:--use-bodhi:" \
        ...
        CUDA_VISIBLE_DEVICES="$GPU" python scripts/eval_healthbench.py \
    ...
    CUDA_VISIBLE_DEVICES="$GPU" python scripts/eval_epistemic.py \
...
python scripts/aggregate_seeds.py --seed-dirs "${SEED_EVAL_DIRS[@]}" \
